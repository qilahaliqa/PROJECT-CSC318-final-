...
if (isset($_POST['psi_t'])) {
  $loginUsername=$_POST['psi_t'];
  $password=$_POST['password'];
  $MM_fldUserAuthorization = "";
  $MM_redirectLoginSuccess = "http://localhost/pustakaseriintan/examples/profile2.php";
  $MM_redirectLoginFailed = "cust_login.php";
  $MM_redirecttoReferrer = false;
  mysql_select_db($database_pustakaseriintan, $pustakaseriintan);
  
  $LoginRS__query=sprintf("SELECT psi_t, password FROM cust_register WHERE psi_t=%s AND password=%s",
              ...
              <div class="panel-body">
                <form id="login-form" class="login" name="login-form" method="POST" action="<?php echo $loginFormAction; ?>">
                  <div class="form-group">
                    <label for="inputID">PSI T</label>
                    <input name="psi_t" type="text" class="form-control" id="psi_t" placeholder="PSI T">
                  </div>
                  <div class="form-group">
...
<br>
									...
									<p>Do not have an account?<a href="cust_reg.php"> Sign Up</a></p>
                </form>
              </div>
            </div>
          </div>
